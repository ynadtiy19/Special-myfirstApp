...
class _CupertinoTabBarState extends State<CupertinoTabBar>
  ...
  double get _offset {
    ...
    double _tabStartingPoint = widget.outerHorizontalPadding / 2.0 +
        _singleWidth * widget.valueGetter();
    double _tabEndingPoint =
        _tabStartingPoint + _singleWidth + widget.outerHorizontalPadding / 2.0;
    double _targetOffset = _scrollControllerOffset;
    if (_tabEndingPoint > _areaEndingPoint - _singleWidth * 0.25) {
      _targetOffset +=
          (_tabEndingPoint - _areaEndingPoint) + _singleWidth * 1.25;
    } else if (_tabStartingPoint < _areaStartingPoint + _singleWidth * 0.25) {
      _targetOffset -=
          (_areaStartingPoint - _tabStartingPoint) + _singleWidth * 1.25;
    }
    _targetOffset = _targetOffset.clamp(0.0,
    ...
    if (!_showSelf) {
      setState(() {
        ...
        if (_shouldCalculateFullDimensions) {
          ...
          _fullWidth = _renderBox.size.width > _maxWidth!
              ? _renderBox.size.width
              : _maxWidth;
          _fullHeight = _renderBox.size.height > _maxHeight!
              ? _renderBox.size.height
              : _maxHeight;
        }
          ...
          Row(
            children: List<Widget>.generate(
                widget.useSeparators
                    ? widget.widgets.length * 2 - 1
                    : widget.widgets.length, (int index) {
              if (!widget.useSeparators || index % 2 == 0) {
                int _trueIndex =
                    !widget.useSeparators ? index : (index / 2.0).floor();
                return _Tab(
                  widget.widgets[_trueIndex],
                  onTap,
                  _trueIndex,
                );
              } else {
